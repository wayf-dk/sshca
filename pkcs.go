...
var (
	...
	pkcs11MechMap = map[string]uint{
		"ssh-ed25519": CKM_EDDSA,
		"ssh-rsa":     pkcs11.CKM_SHA256_RSA_PKCS,
	}
)
...
func (signer hsmSigner) Sign(rand io.Reader, digest []byte) (signature *ssh.Signature, err error) {
	...
	err = p.SignInit(session, []*pkcs11.Mechanism{pkcs11.NewMechanism(pkcs11MechMap[format], nil)}, signer.priv)
	if err != nil {
		log.Fatalf("failed to signinit: %s\n", err)
	}
